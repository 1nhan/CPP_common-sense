...
0. 모호한 아이디어(vague idea)로부터 시작하여 
1. 분석(analysis) 
2. 설계(design) 
3. 구현(implementation) 
4. 테스트(testing) 
5. 재설계(redesign) 
6. 재구현(re-implementation) 단계들을 거쳐 하나의 프로그램을 개발해 나아가는 것이다.

----------------------------------------------------------------

분석(Analysis): 무엇을 해야 하는지를 파악하고 그에 대한 (현재의) 이해를 서술한다. 
이러한 서술을 요구사항 집합(requirements) 또는 명세(specification)라고 한다. 
요구사항이 어떻게 개발되고 문서화되는지는 여기서 다루지 않겠다. 
문제의 크기가 커질수록 그 중요성은 커진다.

설계(Design): 시스템의 전체 구조를 만들고 구현이 가져야 할 부분들과 그 부분들이 
어떻게 통신할지를 결정한다. 설계의 일부로서 어떤 도구들(예: 라이브러리)이 프로그램 
구조화에 도움이 될지 고려한다.

구현(Implementation): 코드를 작성하고 디버그하며 그것이 의도한 대로 동작하는지 
테스트한다.
